...
3. Middleware mới `requirePermission(resource, action)` — gắn vào từng route trong `routes.ts`. Mapping:
   ...
   | (Profit/Dashboard dùng data các API trên → quyền `view` của resource tương ứng; thêm check `profit: view` ở endpoint tổng hợp nếu sau này có) |
...
- `DELETE /api/users/:uid` — xóa cả Firebase user + record (hoặc chỉ disable, mặc định khuyến nghị **disable** để giữ lịch sử; xóa hẳn là hành động riêng).
- Chặn admin tự thao tác lên chính uid có claim admin (không cho disable/xóa admin).
...
2. `App.jsx`:
   - Sidebar: lọc `menuItems` theo `can(resource, 'view')`; thêm mục "Người dùng" + "Lịch sử hoạt động" (chỉ admin, hoặc ai có quyền `activity: view`).
...
## 6. Câu hỏi mở (anh quyết khi duyệt plan)

1. Tab "Người dùng" và "Lịch sử hoạt động" đặt trong Sidebar riêng hay gộp vào trang Cài Đặt? (Plan đang đề xuất: 2 mục sidebar riêng, chỉ hiện với admin.)
2. Dashboard/Lợi Nhuận hiển thị số liệu tổng hợp tiền — có coi là resource riêng cần cấp quyền `view` không, hay ai vào được app đều thấy Dashboard? (Plan đang đề xuất: là resource riêng, cấp được quyền.)
3. Xóa user: xóa hẳn khỏi Firebase hay chỉ disable? (Plan khuyến nghị mặc định disable.)
